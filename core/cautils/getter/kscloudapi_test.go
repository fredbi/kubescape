...
func TestBuildURL(t *testing.T) {
	...
	ks := NewKSCloudAPICustomized("api.example.com", "auth.example.com")

	t.Run("should build URL with query params on https host", func(t *testing.T) {
		require.Equal(t,
			"https://api.example.com/path?q1=v1&q2=v2",
			ks.buildURL("/path", "q1", "v1", "q2", "v2"),
		)
	})

	t.Run("should build URL with query params on http host", func(t *testing.T) {
		ku := NewKSCloudAPICustomized("http://api.example.com", "auth.example.com")

		require.Equal(t,
			"http://api.example.com/path?q1=v1&q2=v2",
			ku.buildURL("/path", "q1", "v1", "q2", "v2"),
		)
	})
	...
	t.Run("should panic when params are not provided in pairs", func(t *testing.T) {
		require.Panics(t, func() {
			...
			_ = ks.buildURL("/path", "q1", "v1", "q2") //nolint:staticcheck
		})
	})
}
