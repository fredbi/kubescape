...
// GetKSCloudAPIConnector returns the KS Cloud client registered for this package.
//
...
func newKSCloudAPI(apiURL, authURL string, opts ...KSCloudOption) *KSCloudAPI {
	...
	api.scheme, api.host = parseHost(apiURL)
...
// Delete exception remove a registered exception rule.
...
func (api *KSCloudAPI) Login() error {
	...
	rdr, _, err := api.post(api.getApiToken(), body, withContentJSON(true))
	if err != nil {
		return err
	}
	defer rdr.Close()
...
func (api *KSCloudAPI) getApiToken() string {
	u := url.URL{}
	u.Scheme, u.Host = parseHost(api.cloudAuthURL)
	u.Path = pathLogin
...
// buildURL builds an URL with query params.
//
// Params are provided in pairs (param name, value).
func (api *KSCloudAPI) buildURL(pth string, pairs ...string) string {
	if len(pairs)%2 != 0 {
...
func (api *KSCloudAPI) getAuthCookie() (*http.Cookie, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("failed to get cookie from %s: status %d", api.getOpenidCustomers(), resp.StatusCode)
	}
